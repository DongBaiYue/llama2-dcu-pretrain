...
case "$DEVICE" in
    dcu)
        ...
        LOG_FILE="logs/pt/llama3-100b_${NNODES}node.log"
        DIST_LOG_DIR="logs/pt/llama3-100b_${NNODES}node.dist"
        VENV_DIR="$PROJECT_ROOT/../py310"
        ;;
    gpu)
        CONFIG_FILE="configs/pt/train_llama3_100b.yaml"
        LOG_FILE="logs/pt/llama3-100b_${NNODES}node_gpu.log"
        DIST_LOG_DIR="logs/pt/llama3-100b_${NNODES}node_gpu.dist"
...
# 并行策略: TP=8, PP=8 固定 (至少 64 卡), 剩余维度由框架自动分配 (实测为 sharding)
