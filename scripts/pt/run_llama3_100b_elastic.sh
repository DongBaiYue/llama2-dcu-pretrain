...
if [ "$DEVICE" = "auto" ]; then
    if command -v nvidia-smi &>/dev/null; then
        ...
        echo "ERROR: 无法自动检测设备类型，请手动指定: run_llama3_100b_elastic.sh [NNODES] [dcu|gpu]" >&2
        exit 1
    fi
fi

# 自动设置 resume_from_checkpoint (弹性续训)
LATEST=$(ls -d checkpoints/pt/llama3-100b/checkpoint-* 2>/dev/null | sort -t- -k2 -n | tail -1 || true)
STEP=$(echo "$LATEST" | grep -oP '\d+$' 2>/dev/null || true)
STEP=${STEP:-0}
CONFIG_FILE="configs/pt/train_llama3_100b_elastic.yaml"

if [ -n "$LATEST" ]; then
    echo "检测到已有checkpoint: $LATEST"
    sed -i "/^resume_from_checkpoint:/d" "$CONFIG_FILE"
    sed -i "/^load_checkpoint_format:/a resume_from_checkpoint: $LATEST" "$CONFIG_FILE"
...
case "$DEVICE" in
    dcu)
        LOG_FILE="logs/pt/llama3-100b_elastic_${NNODES}node_step${STEP}.log"
        DIST_LOG_DIR="logs/pt/llama3-100b_elastic_${NNODES}node_step${STEP}.dist"
...
# 固定 sharding_io (保证 YAML 状态一致)
sed -i "s|^save_checkpoint_format:.*|save_checkpoint_format: sharding_io|" "$CONFIG_FILE"
echo "已设置 save_checkpoint_format=sharding_io"
...
# 分布式配置: 按 NNODES 解析节点列表，并使用参与节点中的第一个作为 rendezvous 地址
if [ "$DEVICE" = "dcu" ]; then
    SLURM_NODE_LIST="${SLURM_JOB_NODELIST:-${SLURM_NODELIST:-}}"
    if [ -n "$SLURM_NODE_LIST" ]; then
        HOSTS="$(scontrol show hostnames "$SLURM_NODE_LIST" | head -n "$NNODES" | paste -sd, -)"
    else
        echo "WARNING: 未检测到 Slurm 节点列表，使用 srun 发现参与节点" >&2
        HOSTS="$(srun -N "$NNODES" -n "$NNODES" --ntasks-per-node=1 hostname | paste -sd, -)"
    fi
    HOST_COUNT="$(echo "$HOSTS" | tr ',' '\n' | wc -l)"
    if [ "$HOST_COUNT" -ne "$NNODES" ]; then
        echo "ERROR: Slurm 分配节点数不足 (需要 $NNODES, 实际 $HOST_COUNT)" >&2
        exit 1
    fi
else
...
if [ -z "$MASTER_ADDR" ]; then
    echo "ERROR: 无法将主节点 '$MASTER_HOST' 解析为 IPv4 地址" >&2
    exit 1
fi
...
if [ "$DEVICE" = "dcu" ]; then
    ...
    srun -N "$NNODES" -n "$NNODES" --ntasks-per-node=1 -w "$HOSTS" \
